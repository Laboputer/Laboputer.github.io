...
일반적인 큐처럼 추가/삭제 연산이 있지만 추가한 순서에 관계없이 삭제(또는 반환)는 가장 우선순위가 높은 데이터를 가져올 수 있습니다. 그것도 아주 빠르게요.

![](https://laboputer.github.io/assets/img/algorithm/ds/08_pq1.PNG)

### 시간복잡도

일반적으로 배열로 저장한 데이터에서 우선순위가 가장 높은 데이터를 뽑기 위해서는 시간복잡도는 O(N)입니다. 물론 우선순위를 높은 데이터를 빠르게 찾아내는 알고리즘도 있습니다. 데이터가 정렬되어 있으면 [이진 탐색(Binary Search)]()로 O(logN)으로 가능합니다. 하지만 배열은 데이터를 추가하는 것은 O(N)이 걸립니다.

우선순위 큐는 추가 연산도, 삭제(우선순위 높은 데이터 탐색) 연산도 모두 O(logN) 입니다.

## 힙 (Heap)
---

사실 우선순위 큐는 힙이라는 트리로된 자료구조로 구현할 수 있습니다. 힙을 그대로 사용하면 우선순위 큐가 될 수 있기 때문에 앞으로는 힙을 기준으로 보시면 됩니다.
...
그리고 완전이진트리(힙 포함)는 특성상 각 인덱스의 의미를 그림처럼 표현하면 배열로 쉽게 구현할 수 있습니다.

- 1번 인덱스부터 N개가를 넣을 수 있다.
- 루트는 [1]에 있다.
- 현재 노드 i를 기준, 왼쪽 자식 노드: [2*i], 오른쪽 자식 노드: [2*i+1] 에 있다.
