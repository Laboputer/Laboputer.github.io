...
> LINK : https://leetcode.com/contest/weekly-contest-231
...
[Leetcode](https://leetcode.com/)에서의 처음 콘테스트를 해봤습니다. 막상 1시간 30분인 제한시간 안에 풀어보려다보니 잘못된 솔루션도 많이 생각하고 헛코드도 많이 짠 것 같습니다. 콘테스트 시간 동안에는 4문제 중 2문제 밖에 못풀었습니다. 끝나고나니 3번째 문제는 비교적 금방 풀었는데 4번째 문제는 혼자 힘으로는 생각하기 어려워서 결국 Discuss에서 도움을 좀 받았습니다. 역시 갓님들의 아이디어를 보고 많이 배웁니다. 아이디어를 보고 곰곰히 생각해보니 이해가 되어서 그래도 다행입니다.
...
먼저 끝점은 `n`은 고정되어 있으므로 모든 노드로 부터 `n` 까지의 최단거리를 Dijkstra 알고리즘으로 계산합니다. MinHeap Dijkstra 을 이용하여 O((N+E)logN) 로 구할 수 있습니다.
...
정수형 배열 `nums` 와 정수 `k` 가 주어집니다. `nums`에서 어떤 인덱스에 있는 값을 원하는 값으로 변경할 수 있습니다. 이 때 `nums`에서 모든 연속된 `k`개를 XOR한 값을 0으로 만들어야 합니다. 다시 말하면 `[i, i+k-1] where 0 <= i <= nums.size() - k`
구간에 대하여 XOR한 값인 `(nums[i] ^ nums[i+1] ... ^ nums[i+k-1]) = 0` 이 되기 위해 변경해야 하는 최소 횟수를 구하는 문제입니다.

### 풀이

먼저 모든 연속된 `k` 개의 XOR한 값을 0으로 만들어야 하는 것으로 부터
