...
그래프의 일종으로 사이클이 없으며, 모든 정점이 연결되어 있는 것이다.

**트리의 특징**
1. 모든 정점이 연결되어있고 사이클이 없으므로, N-1개 간선이 존재한다.
2. 모든 정점이 연결되어있기 때문에 임의의 두 정점을 선택해도 단순경로가 존재한다.
3. 임의의 간선을 제거하면, 연결이 끊어진다.
4. 임의의 간선을 추가하면, 사이클이 생긴다.
