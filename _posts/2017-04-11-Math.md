...
### 소수 판별
먼저 임의의 수 x가 주어졌을 때 약수를 구하는 방법은 1~sqrt(x) 까지 나누어지는 지를 판단하면 된다. 약수는 쌍으로 존재하기 때문에 나누어질 경우 그 쌍을 알게 된다.

같은 논리로 임의의 수 x가 주어졌을 때 소수를 판별하는 방법은 1 ~ sqrt(x) 까지 수가 모두 나누어지지 않으면 된다.

Time Complexity : O (sqrt(n))

### 에라토스테네스의 체 (Sieve of Eratosthenes)
하나의 소수를 판별하는데는 sqrt(n)이 걸리지만, 특정 범위의 소수들을 모두 구하고자 할 때는 에라토스테네스의 체로 빠르게 구할 수 있다. 에라토스테네스의 체 또한 최적화 방법이 있지만, 간단하게 구해도 n개의 범위의 소수를 구하는데 O(N)에 가까운 복잡도를 가진다.

일반적인 방법에서 조금 더 최적화를 하는 것을 간단하게 소개하면
1. 지워지지 않는 수를 탐색할 떄 N까지가 아니라 sqrt(N) 까지만 구한다.
2. 지워지지 않는 수를 찾았을 때 i+i가 아니라 i*i 부터 탐색한다.

번외로, 에라토스테네스의 체를 이용하면 특정 범위내의 소인수 분해도 빠르게 구할 수 있는 방법이 존재한다.
중요한 아이디어는 특정 수의 소수여부를 저장하는 것이 아니라 가장 작은 소인수를 저장하는 것이다. 이는 에라토스테네스의 체에서 쉽게 수행할 수 있고, 가장 작은 소수를 나눈 값을 반복하는 것으로 빠르게 구할 수 있다.
...
```
	...
	for (int i = 3; i <= MAXN; i += 2) // MAXN 대신 sqrt(n)
		if (prime[i]) for (int j = i + i; j <= MAXN; j += i) prime[j] = 0; // i+i 대신 i*i
```
